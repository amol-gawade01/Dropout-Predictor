...
function StudentDashboard({ setPage }) {
  ...
  const academics = data.academic_summary || {};
  const kpis = [
    ['Average mastery', learning.average_mastery_percentage, '%', Target],
    ['Mastered concepts', learning.mastered_concepts, '', Check],
    ['Practice queue', learning.concepts_needing_practice, '', BookOpen],
    ['Completed sessions', learning.completed_sessions, '', Activity],
  ];
  return <><div className="welcome-row"><div><p className="eyebrow">Your learning pulse</p><h2>Keep your momentum, one concept at a time.</h2><p className="muted">Your workspace updates as you learn. Focused practice is already waiting.</p></div><button className="button primary" onClick={() => setPage('learn')}><Sparkles size={16} /> Start learning</button></div>{state.error && <Notice text={state.error} />}<div className="kpi-grid">{kpis.map(([label, value, suffix, Icon]) => <Metric key={label} label={label} value={formatNumber(value, suffix)} icon={Icon} loading={state.loading} />)}</div><section className="academic-section"><div className="section-heading"><div><p className="eyebrow">Academic snapshot</p><h3>Attendance and results</h3></div>{academics.week_start_date && <span className="muted">Updated {new Date(academics.week_start_date).toLocaleDateString()}</span>}</div>{academics.available ? <div className="academic-grid"><div className="academic-card attendance-card"><Activity size={19} /><span>Attendance</span><strong>{formatNumber(academics.attendance_percentage, '%')}</strong><small className={academics.attendance_change_14d < 0 ? 'negative' : 'positive'}>{academics.attendance_change_14d > 0 ? '+' : ''}{formatNumber(academics.attendance_change_14d, '%')} over 14 days</small></div><div className="academic-card"><GraduationCap size={19} /><span>Current GPA</span><strong>{formatNumber(academics.current_gpa)}</strong><small>Semester {academics.semester ?? '—'}</small></div><div className="academic-card"><Check size={19} /><span>Credits completed</span><strong>{formatNumber(academics.credits_completion_percentage, '%')}</strong><small>{academics.failed_subjects ?? 0} failed subjects</small></div><div className="academic-card"><BookOpen size={19} /><span>Assignments</span><strong>{formatNumber(academics.assignment_completion_percentage, '%')}</strong><small>{academics.missed_assessments ?? 0} missed assessments</small></div></div> : !state.loading && <div className="panel"><EmptyState icon={GraduationCap} title="No academic result yet" text={academics.message || 'Your institution has not uploaded attendance or results yet.'} /></div>}</section><div className="dashboard-grid"><section className="panel support-panel"><div className="panel-heading"><div><p className="eyebrow">Support status</p><h3>{data.support?.level?.replaceAll('_', ' ') || 'Your learning plan is ready'}</h3></div><span className={`status-dot ${String(data.support?.level || 'ON_TRACK').toLowerCase()}`} /></div><p className="muted">{data.support?.faculty_support_status ? 'Your faculty support plan is ' + data.support.faculty_support_status.toLowerCase().replaceAll('_', ' ') + '.' : 'Complete a practice session to keep your progress visible here.'}</p><div className="status-line"><span>Current pathway</span><strong>{data.pathway || 'Personalized practice'}</strong></div></section><section className="panel next-panel"><div className="panel-heading"><div><p className="eyebrow">Recommended next</p><h3>Build confidence where it counts</h3></div><ArrowUpRight size={18} /></div>{data.recommended_next?.length ? data.recommended_next.slice(0, 2).map((concept) => <div className="recommendation" key={concept.concept_id || concept.id}><div className="concept-icon"><BrainCircuit size={18} /></div><div><strong>{concept.topic_name || concept.topic || concept.name}</strong><p>{concept.reason || 'Recommended for your next practice session.'}</p></div><button className="text-button" onClick={() => setPage('learn')}>Learn <ChevronRight size={15} /></button></div>) : <EmptyState icon={BrainCircuit} title="Recommendations will appear here" text="Your next concept will be selected from backend learning signals." />}</section></div><section className="panel"><div className="section-heading"><div><p className="eyebrow">Recent activity</p><h3>Learning sessions</h3></div><button className="text-button" onClick={() => setPage('sessions')}>View history <ArrowUpRight size={15} /></button></div>{data.recent_sessions?.length ? <div className="table-wrap"><table><thead><tr><th>Session</th><th>Progress</th><th>Result</th><th>Status</th></tr></thead><tbody>{data.recent_sessions.map((session, i) => <tr key={session.session_id || i}><td><strong>{session.concept_name || session.topic || 'Learning session'}</strong><small>{session.created_at ? new Date(session.created_at).toLocaleDateString() : 'Recent'}</small></td><td>{session.answered ?? 0} / {session.target_questions ?? session.target ?? '—'} answered</td><td>{session.correct ?? 0} / {session.answered ?? 0} correct</td><td><span className="table-status">{session.status || 'IN PROGRESS'}</span></td></tr>)}</tbody></table></div> : <EmptyState icon={Activity} title="No sessions yet" text="Start a learning session to see your progress build here." action="Start learning" onAction={() => setPage('learn')} />}</section></>;
}
...
function FacultyStudentDetail({ data }) {
  ...
  return <><div className="welcome-row"><div><p className="eyebrow">Faculty student detail</p><h2>{student.display_name || riskData.display_name || 'Student profile'}</h2><p className="muted">{student.student_code || riskData.student_code || '—'} · {student.program_stream || '—'}</p></div><span className={`risk-pill ${risk.risk_tier?.toLowerCase()}`}>{risk.risk_tier || 'NOT EVALUATED'} {risk.risk_percentage == null ? '' : `· ${risk.risk_percentage}%`}</span></div><div className="detail-grid"><section className="panel"><p className="eyebrow">Top Predictive Risk Factors</p>{risk.top_risk_factors?.length ? risk.top_risk_factors.map((factor, index) => <div className="insight-item" key={factor.factor || index}><div><strong>{displayValue(factor.factor)}</strong><small>Predictive driver</small></div><b>{displayValue(factor.contribution_percentage)}%</b></div>) : <EmptyState icon={ShieldCheck} title="No predictive factors" text="Risk factors appear after evaluation." />}</section><section className="panel"><div className="section-heading"><div><p className="eyebrow">Risk history</p><h3>Evaluation snapshots</h3></div></div>{history.length ? <div className="chart-wrap"><ResponsiveContainer width="100%" height={220}><LineChart data={history}><XAxis dataKey="week_start_date" /><YAxis domain={[0, 100]} unit="%" /><Tooltip /><Line type="monotone" dataKey="risk_percentage" stroke="#c94e4e" strokeWidth={3} /></LineChart></ResponsiveContainer></div> : <EmptyState icon={Activity} title="No risk history" text="Evaluate this student to create a snapshot." />}</section></div><GuardianReport code={student.student_code || riskData.student_code} /><section className="panel"><p className="eyebrow">Mastery by concept</p>{mastery.length ? <div className="mastery-grid">{mastery.map((item) => <div className="mastery-card" key={item.concept_id}><h3>{item.topic_name || item.concept_name}</h3><div className="mastery-track"><div style={{ width: `${item.mastery_percentage || 0}%` }} /></div><strong>{item.mastery_percentage ?? 0}%</strong></div>)}</div> : <EmptyState icon={Target} title="No mastery data" text="This student's learning activity will appear here." />}</section></>;
}
function GuardianReport({ code }) {
  const [state, setState] = useState({ loading: true, data: null, note: '', busy: false, message: '' });
  useEffect(() => { if (code) facultyApi.reportDetails(code).then(({ data }) => setState((s) => ({ ...s, loading: false, data: unwrap(data) }))).catch((e) => setState((s) => ({ ...s, loading: false, message: apiErrorMessage(e, 'Could not load guardian details.') }))); }, [code]);
  const preview = async () => { try { const response = await facultyApi.reportPreview(code); window.open(URL.createObjectURL(response.data), '_blank', 'noopener,noreferrer'); } catch (e) { setState((s) => ({ ...s, message: apiErrorMessage(e, 'Could not generate PDF.') })); } };
  const send = async (guardian) => { if (!window.confirm(`Email the PDF report to ${guardian.guardian_name} at ${guardian.email_address}?`)) return; setState((s) => ({ ...s, busy: true, message: '' })); try { const { data } = await facultyApi.sendReport(code, { guardian_id: guardian.guardian_id, mentor_note: state.note || null }); setState((s) => ({ ...s, busy: false, message: data.message })); } catch (e) { setState((s) => ({ ...s, busy: false, message: apiErrorMessage(e, 'Report delivery failed.') })); } };
  return <section className="panel report-panel"><div className="section-heading"><div><p className="eyebrow">Guardian report</p><h3>Email progress PDF to guardian</h3></div><button className="button secondary small" disabled={!state.data?.report_available} onClick={preview}>Preview PDF</button></div>{state.message && <Notice text={state.message} />}<label className="report-note">Optional faculty note<textarea maxLength="1000" value={state.note} onChange={(e) => setState((s) => ({ ...s, note: e.target.value }))} placeholder="Add a constructive note for the guardian" /></label>{state.loading ? <LoadingScreen text="Loading guardians..." /> : state.data?.guardians?.length ? <div className="guardian-list">{state.data.guardians.map((g) => <div className="guardian-row" key={g.guardian_id}><div><strong>{g.guardian_name}</strong><small>{g.relationship} · {g.email_address || 'No email'} · {g.email_opt_in ? 'Consent recorded' : 'No consent'}</small></div><button className="button primary small" disabled={state.busy || !g.email_address || !g.email_opt_in || !state.data.report_available} onClick={() => send(g)}>{state.busy ? 'Sending...' : 'Email PDF'}</button></div>)}</div> : <EmptyState icon={Users} title="No guardian linked" text="Add an opted-in guardian email before sending reports." />}</section>;
}
